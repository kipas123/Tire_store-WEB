...
@Named(value = "orderToHandleBean")
@ViewScoped
public class OrderToHandleBean implements Serializable {
	...
	private List<Storeorder> getResultList(int offset, int pageSize, String search) {
		if (search != null)
			return storeorderDAO.getOrdersToHandle(search, offset, pageSize);
		return storeorderDAO.getOrdersToHandle(offset, pageSize);
	}

	private int count(String search) {
		if (this.search != null)
			return storeorderDAO.countOrdersToHandle(search);
		return storeorderDAO.countOrdersToHandle();
	}
}
