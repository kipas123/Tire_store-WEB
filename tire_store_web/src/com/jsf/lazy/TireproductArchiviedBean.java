...
@Named(value = "tireproductArchiviedBean")
@ViewScoped
public class TireproductArchiviedBean implements Serializable {
	 ...
	 */
	public TireproductArchiviedBean() {
	...
	private List<Tireproduct> getResultList(int offset, int pageSize, String search) {
		if (search != null)
			return tireproductDAO.getProductsArchived(search, offset, pageSize);
		return tireproductDAO.getProductsArchived(offset, pageSize);
	}

	private int count(String search) {
		if (this.search != null)
			return tireproductDAO.countProductsArchived(search);
		return tireproductDAO.countProductsArchived();
	}
