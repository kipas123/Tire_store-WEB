...
public class CrudView implements Serializable {
    ...
    public void saveProduct() {
    	...
    	if(this.selectedProduct.getBrand()!=null && this.selectedProduct.getModel()!=null && this.selectedProduct.getSize()!=null && this.selected>0) {
    		this.selectedProduct.setTiretype(tiretypeDAO.get(this.selected));
    		tireproductDAO.insert(this.selectedProduct);
    		ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Dodano produkt!", null));
            PrimeFaces.current().executeScript("PF('manageProductDialog').hide()");
            PrimeFaces.current().ajax().update("form:messages", "form:dt-products");
    	}else {
    		ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Podaj wszystkie wymagane pola!", null));
    	}
    	}else {
    		if(this.selectedProduct.getBrand()!=null && this.selectedProduct.getModel()!=null && this.selectedProduct.getSize()!=null && this.selected>0) {
        		this.selectedProduct.setTiretype(tiretypeDAO.get(this.selected));
        		tireproductDAO.update(this.selectedProduct);
        		ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Zaaktualizowano produkt!", null));
                PrimeFaces.current().executeScript("PF('manageProductDialog').hide()");
                PrimeFaces.current().ajax().update("form:messages", "form:dt-products");
    	}else {
    		ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Podaj wszystkie wymagane pola!", null));
